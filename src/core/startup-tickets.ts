...
const STARTUP_TICKET_DEFS: StartupTicketDef[] = [
    ...
    {
        order: 2,
        title: 'Build L2 branch head groups',
        body: [
            '**Objective:** Create 6 L2 branch head groups under the existing L1 orchestrator.',
            '',
            '**Context:** L0 Boss and L1 GlobalOrchestrator already exist (created by ensureDefaultTree).',
            'This ticket creates the v10.0 group-based overlay starting at L2.',
            '',
            '**Steps:**',
            '1. Find the existing L1 head node (GlobalOrchestrator) from ensureDefaultTree.',
            '2. Create 6 L2 groups (one per branch: Planning, Verification, Coding, Co-Director, Data, Orchestrator).',
            '3. Each group gets 10 slots: HeadOrchestrator + 9 mandatory roles.',
            '4. Wire parent-child relationships to existing L1 head.',
            '',
            '**Success criteria:** 6 L2 branch head groups created. All linked to existing L1.',
        ].join('\n'),
        priority: TicketPriority.P1,
        category: 'system_bootstrap',
        depends_on: [1],
