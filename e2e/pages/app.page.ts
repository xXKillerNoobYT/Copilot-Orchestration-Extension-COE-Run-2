...
export class AppPage {
    ...
    async navigateToTab(tab: TabName) {
    ...
    async openModal(modalId: string) {
        await this.page.evaluate((id) => {
            const fn = (window as any).openModal;
            if (fn) fn(id);
        }, modalId);
    }
    ...
    async getDashboardCards(): Promise<Array<{ value: string; label: string }>> {
